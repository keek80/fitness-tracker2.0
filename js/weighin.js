...
function renderWeighInChart(weighIns, settings) {
    ...
    // Build projected line (from start to 104 weeks)
    const projLabels = [];
    const projData = [];
    for (let w = 0; w <= 104; w += 4) {
        projLabels.push(`Wk ${w}`);
        projData.push(Storage.getProjectedWeight(w));
    }

    // Build actual line
    const actualLabels = weighIns.map(e => formatDateShort(e.date));
