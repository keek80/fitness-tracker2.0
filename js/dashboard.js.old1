...
function renderDashboard() {
    ...
    const scheduledWorkoutDay = program.days.find(d => d.dayOfWeek === todayDayOfWeek);
    const effectiveId = (typeof getEffectiveWorkout === 'function')
        ? getEffectiveWorkout(todayStr)
        : (scheduledWorkoutDay ? scheduledWorkoutDay.id : null);
    const todaysWorkoutDay = effectiveId ? program.days.find(d => d.id === effectiveId) : null;
    const isSwapped = !!(todaysWorkoutDay && scheduledWorkoutDay && todaysWorkoutDay.id !== scheduledWorkoutDay.id);
    const todayAlreadyLogged = todaysWorkoutDay
        ? gymLogs.some(l => l.date === todayStr && l.dayId === todaysWorkoutDay.id)
        : gymLogs.some(l => l.date === todayStr);

    const otherDays = program.days.filter(d => !todaysWorkoutDay || d.id !== todaysWorkoutDay.id);
    const swapPicker = otherDays.length ? `
        <div class="dash-swap-wrap">
            <button class="dash-swap-toggle" type="button" onclick="event.stopPropagation(); toggleDashboardSwapPicker()">
                🔄 Change today's session
            </button>
            <div id="dashSwapPicker" class="dash-swap-picker hidden">
                ${scheduledWorkoutDay && isSwapped ? `
                    <button class="dash-swap-option" type="button" onclick="event.stopPropagation(); pickDashboardWorkout('${scheduledWorkoutDay.id}')">
                        ↩ Back to ${scheduledWorkoutDay.name} (${scheduledWorkoutDay.dayOfWeek.slice(0,3)})
                    </button>
                ` : ''}
                ${otherDays.map(d => `
                    <button class="dash-swap-option" type="button" style="border-left:3px solid ${d.color}"
                            onclick="event.stopPropagation(); pickDashboardWorkout('${d.id}')">
                        ${d.dayOfWeek.slice(0,3)} · ${d.name}
                    </button>
                `).join('')}
            </div>
        </div>
    ...
    let todayCard = '';
    if (todaysWorkoutDay) {
        const subtitle = isSwapped
            ? `Swapped — scheduled was ${scheduledWorkoutDay.name} · ${todaysWorkoutDay.exercises.length} exercises`
            : `${todayDayOfWeek} — ${todaysWorkoutDay.exercises.length} exercises`;
        if (todayAlreadyLogged) {
            ...
            todayCard = `
                <div class="card today-workout-card" style="border-left: 3px solid ${todaysWorkoutDay.color}; padding: 12px 14px; cursor:pointer" onclick="navigate('gym')">
                    <div class="flex-between">
                        <div>
                            <div style="font-size:12px; color:${todaysWorkoutDay.color}; font-weight:600">${isSwapped ? '🔄 TODAY\'S WORKOUT (SWAPPED)' : '📅 TODAY\'S WORKOUT'}</div>
                            <div style="font-size:15px; font-weight:700; margin-top:4px">${todaysWorkoutDay.name}</div>
                            <div style="font-size:12px; color:var(--text-muted)">${subtitle}</div>
                        </div>
            ...
            <div class="card" style="border-left: 3px solid var(--text-muted); padding: 12px 14px;">
                <div class="flex-between">
                    <div>
                        ...
                        <div style="font-size:12px; color:var(--text-muted)">Active recovery: 20-30 min walk, stretching, foam rolling — or pick a session below.</div>
                    </div>
                    <div style="font-size:32px">🧘</div>
                </div>
                ${swapPicker}
...
function toggleDashboardSwapPicker() {
    const picker = document.getElementById('dashSwapPicker');
    if (!picker) return;
    picker.classList.toggle('hidden');
}

function pickDashboardWorkout(dayId) {
    const todayStr = getLocalDateString();
    if (typeof applyWorkoutOverride === 'function') {
        applyWorkoutOverride(todayStr, dayId);
    } else if (typeof Storage !== 'undefined') {
        Storage.setWorkoutOverride(todayStr, dayId);
    }
    if (typeof currentGymDay !== 'undefined') currentGymDay = dayId;
    if (typeof currentGymDate !== 'undefined') currentGymDate = todayStr;
    const program = getTrainingProgram();
    const day = program.days.find(d => d.id === dayId);
    showToast(day ? `Today's session set to ${day.name}` : 'Session updated');
    renderDashboard();
}
