...
function confirmSaveProgram() {
    ...
    if (saveCurrentProgramAs(name) && downloadProgramJson(name)) {
        closeExModal();
        showToast(`Saved "${name}" JSON to this device`);
    } else {
    ...
    if (isCustomProgram()) {
        ...
        if (saveFirst) {
            ...
            if (name && name.trim()) {
                ...
                downloadProgramJson(name.trim());
                showToast(`Saved "${name.trim()}" JSON to this device`);
            } else {
...
function pickSavedProgramFile() {
    const input = document.getElementById('importWorkoutFile');
    if (!input) {
        showToast('File picker not ready', 'error');
        return;
    }
    input.click();
}
...
function openLoadProgramModal() {
    ...
    const fileBtn = `
        <button class="btn btn-success" style="width:100%; margin-bottom:14px" onclick="pickSavedProgramFile()">
            📂 Choose JSON file on this device
        </button>
        <div style="font-size:12px; color:var(--text-muted); margin-bottom:14px; text-align:center">
            Pick a program JSON you saved earlier (Downloads / Files).
        </div>
    `;

    const list = names.length === 0 ? `
        <div style="text-align:center; padding:8px 0 4px; color:var(--text-muted); font-size:13px">
            No programs cached in the browser yet. Use the button above to load a JSON file.
        </div>
    ` : `
        <div style="font-size:13px; color:var(--text-muted); margin-bottom:10px">
            Or load one cached in this browser:
        </div>
        <div style="display:flex; flex-direction:column; gap:8px">
            ${names.map(name => {
                const dayCount = programs[name].days ? programs[name].days.length : 0;
    ...
    openExModal('📂 Load Saved Program', fileBtn + list);
}
...
function buildProgramExportPayload(name, program) {
    return {
...
function downloadProgramJson(name) {
    const program = getTrainingProgram();
    if (!program || !Array.isArray(program.days) || program.days.length === 0) {
        showToast('No workout to save', 'error');
        return false;
    }
    const label = name || getActiveProgramName() || 'current-workout';
    const stamp = typeof getLocalDateString === 'function' ? getLocalDateString() : new Date().toISOString().slice(0, 10);
    const filename = slugifyWorkoutName(label) + '-' + stamp + '.json';
    downloadFile(filename, JSON.stringify(buildProgramExportPayload(label, program), null, 2), 'application/json');
    return true;
}
...
function downloadCurrentWorkout() {
    ...
    downloadFile(slug + '-' + stamp + '.json', JSON.stringify(buildProgramExportPayload(name, program), null, 2), 'application/json');
...
function importWorkoutFile(event) {
    ...
    reader.onload = function (e) {
        try {
            ...
            if (!confirm('Replace the current workout with "' + (data.name || file.name) + '"?')) return;
            saveTrainingProgram(program);
            const label = data.name ? String(data.name) : file.name.replace(/\.json$/i, '');
            if (label) {
                setActiveProgramName(label);
                const programs = getSavedPrograms();
                programs[label] = JSON.parse(JSON.stringify(program));
                saveSavedPrograms(programs);
            }
            closeExModal();
            renderExercises();
            showToast('Loaded program from ' + file.name);
        } catch (err) {
            console.error(err);
