...
function getEffectiveWorkout(dateStr) {
    const overrideId = Storage.getWorkoutOverride(dateStr);
    const program = getTrainingProgram();
    if (overrideId && program.days && program.days.find(d => d.id === overrideId)) {
        return overrideId;
    }
...
function applyWorkoutOverride(dateStr, dayId) {
    ...
    if (!dayId || dayId === scheduled) {
        Storage.clearWorkoutOverride(dateStr);
    } else {
        Storage.setWorkoutOverride(dateStr, dayId);
    }
}

function renderSessionChips(program, activeId, scheduledId, clickFn) {
    if (!program.days || program.days.length === 0) return '';
    return `<div class="day-selector">
        ${program.days.map(d => {
            const isActive = d.id === activeId;
            const isScheduled = d.id === scheduledId;
            const textColor = isActive ? (isLightColor(d.color) ? '#000' : '#fff') : '';
            return `<button type="button" class="day-btn ${isActive ? 'active' : ''} ${isScheduled && !isActive ? 'scheduled' : ''}"
...
function renderGym() {
    ...
    const overrideId = Storage.getWorkoutOverride(currentGymDate);
    ...
    const isOverride = !!(overrideId && currentGymDay && overrideId === currentGymDay && overrideId !== scheduledWorkout);
    let scheduleIndicator = '';

    if (isTodayDate && isOverride) {
        const instead = scheduledDay ? ` instead of ${scheduledDay.name}` : '';
        scheduleIndicator = `<div class="today-workout-banner"><span class="today-badge">📅 TODAY</span><span>It's ${selectedDayName} — logging <strong>${day.name}</strong>${instead}</span></div>`;
    } else if (isTodayDate && scheduledWorkout && currentGymDay === scheduledWorkout) {
    ...
    } else if (isTodayDate && !scheduledWorkout) {
        ...
        const extra = isOverride && scheduledDay ? ` · ${day.name} instead of ${scheduledDay.name}` : '';
        scheduleIndicator = `<div class="past-date-banner"><span>📆 ${formatDate(currentGymDate)} (${selectedDayName})${extra}</span></div>`;
    }
    ...
    page.innerHTML = `
        ...
        ${scheduleIndicator}
        ${renderSessionChips(program, currentGymDay, scheduledWorkout, 'selectGymDay')}
        ...
        <div class="card" style="border-left: 3px solid ${day.color}; padding: 12px 14px;">
            ...
            <div style="font-size:12px; color:var(--text-muted)">${selectedDayName} · ${day.exercises.length} exercises${isOverride ? ' · usually ' + day.dayOfWeek : ''}</div>
        </div>
...
function clearGymDayOverride() {
    Storage.clearWorkoutOverride(currentGymDate);
