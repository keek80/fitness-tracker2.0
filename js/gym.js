...
function applyWorkoutOverride(dateStr, dayId) {
    ...
    if (!sourceDate) {
        ...
        if (session) sourceDate = dates.find(ds => weekdayNameFromDate(ds) === session.dayOfWeek);
    }
...
function renderSessionChips(program, activeId, viewDate, clickFn) {
    ...
    return `<div class="day-selector">
        ${program.days.map(d => {
            ...
            const shownWeekday = getEffectiveWeekdayForSession(d.id, viewDate) || d.dayOfWeek;
            const isTodaySlot = d.id === todayId;
            const textColor = isActive ? (isLightColor(d.color) ? '#000' : '#fff') : '';
            return `<button type="button" class="day-btn ${isActive ? 'active' : ''} ${isTodaySlot && !isActive ? 'scheduled' : ''}"
