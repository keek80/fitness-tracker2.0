...
deleteGymLog(date, dayId) {
    ...
    // ===== THIS-WEEK SESSION SWAPS (temporary, not the program) =====
    getWeekSwaps() { return this.get('week_swaps', {}); },

    getWeekSwapMap(weekStart) {
        const all = this.getWeekSwaps();
        return (all && all[weekStart]) ? { ...all[weekStart] } : {};
    },

    setWeekSwapMap(weekStart, map) {
        const all = { ...this.getWeekSwaps() };
        if (!map || Object.keys(map).length === 0) delete all[weekStart];
        else all[weekStart] = map;
        this.set('week_swaps', all);
        return all;
    },

    // Legacy single-date key still read as a fallback
    getWorkoutOverrides() { return this.get('workout_overrides', {}); },

    getWorkoutOverride(dateStr) {
        const weekStart = (typeof getWeekStartDate === 'function') ? getWeekStartDate(dateStr) : dateStr;
        const weekMap = this.getWeekSwapMap(weekStart);
        if (Object.prototype.hasOwnProperty.call(weekMap, dateStr)) {
            return weekMap[dateStr] || null;
        }
        const overrides = this.getWorkoutOverrides();
        return overrides && overrides[dateStr] ? overrides[dateStr] : null;
    },

    setWorkoutOverride(dateStr, dayId) {
        const overrides = { ...this.getWorkoutOverrides() };
        if (!dayId) delete overrides[dateStr];
        else overrides[dateStr] = dayId;
        this.set('workout_overrides', overrides);
        return overrides;
    },

    clearWorkoutOverride(dateStr) {
        return this.setWorkoutOverride(dateStr, null);
    },
    ...
    exportAll() {
        return JSON.stringify({
            ...
            workoutOverrides: this.getWorkoutOverrides(),
            weekSwaps: this.getWeekSwaps(),
    ...
    importAll(jsonString) {
        try {
            ...
            if (data.workoutOverrides && typeof data.workoutOverrides === 'object' && !Array.isArray(data.workoutOverrides)) {
                const safeOverrides = {};
                Object.entries(data.workoutOverrides).forEach(([date, dayId]) => {
                    if (typeof date === 'string' && typeof dayId === 'string' && dayId) {
                        safeOverrides[date] = dayId;
                    }
                });
                this.set('workout_overrides', safeOverrides);
            }

            if (data.weekSwaps && typeof data.weekSwaps === 'object' && !Array.isArray(data.weekSwaps)) {
                const safeWeeks = {};
                Object.entries(data.weekSwaps).forEach(([weekStart, map]) => {
                    if (typeof weekStart !== 'string' || !map || typeof map !== 'object') return;
                    const safeMap = {};
                    Object.entries(map).forEach(([date, dayId]) => {
                        if (typeof date === 'string' && (typeof dayId === 'string' || dayId === null)) {
                            safeMap[date] = dayId || '';
                        }
                    });
                    if (Object.keys(safeMap).length) safeWeeks[weekStart] = safeMap;
                });
                this.set('week_swaps', safeWeeks);
            }
