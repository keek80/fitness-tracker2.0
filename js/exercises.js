                    ...
                    <div class="ex-day-header">
                        ...
                        <div class="ex-day-actions">
                            <select class="form-input" style="width:auto; font-size:12px; padding:4px 8px"
                                    ...
                                    title="Permanently assign this session to a weekday">
                                ${['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday']
                                    .map(d => `<option value="${d}" ${d === day.dayOfWeek ? 'selected' : ''}>${d.slice(0,3)}</option>`).join('')}
                            </select>
    ...
    openExModal('Edit Training Day', `
        ...
        <div class="form-group">
            ...
            <div style="font-size:12px; color:var(--text-muted); margin-top:6px">
                If another session already uses that weekday, the two days will swap. This changes the program permanently.
            </div>
        </div>
        <div class="form-group">
            <label class="form-label">Color</label>
...
function saveDayForm() {
    ...
    if (editingDayId) {
        ...
        if (day) {
            swapProgramWeekdays(newProgram, editingDayId, dayOfWeek);
            day.name = name;
...
function swapProgramWeekdays(program, dayId, newWeekday) {
    const day = program.days.find(d => d.id === dayId);
    if (!day || day.dayOfWeek === newWeekday) return;
    const occupant = program.days.find(d => d.id !== dayId && d.dayOfWeek === newWeekday);
    if (occupant) occupant.dayOfWeek = day.dayOfWeek;
...
function reassignDayWeekday(dayId, newWeekday) {
    ...
    const occupant = program.days.find(d => d.id !== dayId && d.dayOfWeek === newWeekday);
    const newProgram = JSON.parse(JSON.stringify(program));
    swapProgramWeekdays(newProgram, dayId, newWeekday);
    saveTrainingProgram(newProgram);
    renderExercises();
