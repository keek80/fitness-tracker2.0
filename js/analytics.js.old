...
function getExerciseTrend(entries) {
    ...
    // Compare by weight first, then by volume
...
function renderExerciseDetail(name, entries, dayColor) {
    ...
    // Use the heaviest set's reps
    const maxSingleSetReps = Math.max(...latest.sets.filter(s => s > 0));
    ...
    return `
        ...
        <div class="card" style="border-left: 3px solid ${dayColor}; margin-top: 14px">
            ...
            ${pr ? `<div class="pr-badge" style="margin-top:6px; display:inline-block">🏆 PR: ${pr.bestWeight} lbs × ${pr.bestReps} reps</div>` : ''}
        </div>
...
function renderPRAnalytics(container, prs) {
    ...
    container.innerHTML = `
        ...
        ${prList.sort((a,b) => b[1].bestVolume - a[1].bestVolume).map(([name, pr]) => `
            <div class="card" style="border-left: 3px solid #ffd700">
                <div class="flex-between">
                    <div>
                        <div style="font-size:14px; font-weight:600">${name}</div>
                        <div style="font-size:12px; color:var(--text-muted)">${formatDate(pr.date)}</div>
                    </div>
                    <div style="text-align:right">
                        <div style="font-size:18px; font-weight:700; color:#ffd700">${pr.bestWeight} lbs</div>
                        <div style="font-size:12px; color:var(--text-secondary)">${pr.bestReps} total reps · ${pr.bestVolume.toLocaleString()} vol</div>
                    </div>
                </div>
            </div>
        `).join('')}
...
function renderStatsAnalytics(container, weighIns, gymLogs, settings) {
    ...
    // Workouts per week average (divide by weeks, not days)
    let workoutsPerWeek = '—';
    if (gymLogs.length >= 2) {
