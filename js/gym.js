...
function getWeekStartDate(dateStr) {
    ...
    return shiftLocalDate(dateStr, offset);
}

function getWeekDates(dateStr) {
    const start = getWeekStartDate(dateStr);
    return [0,1,2,3,4,5,6].map(i => shiftLocalDate(start, i));
}

function weekdayNameFromDate(dateStr) {
    return DAY_NAME_MAP[new Date(dateStr + 'T12:00:00').getDay()];
}

function isAlternateDay(day) {
    if (!day) return false;
    return day.alternate === true || day.dayOfWeek === 'Alternate' || day.dayOfWeek === 'Alt';
}

function getCycleDays(program) {
    return (program.days || []).filter(d => !isAlternateDay(d));
}

function getAlternateDays(program) {
    return (program.days || []).filter(d => isAlternateDay(d));
}
...
function getScheduledWorkout(dateStr) {
    ...
    const dayOfWeek = weekdayNameFromDate(dateStr);
    const match = getCycleDays(program).find(d => d.dayOfWeek === dayOfWeek);
    return match ? match.id : null;
}

function getTodaysWorkout(dateStr) {
    return getEffectiveWorkout(dateStr);
}

function getEffectiveWorkout(dateStr) {
    const program = getTrainingProgram();
    const weekMap = Storage.getWeekSwapMap(getWeekStartDate(dateStr));
    if (Object.prototype.hasOwnProperty.call(weekMap, dateStr)) {
        const overrideId = weekMap[dateStr];
        if (!overrideId) return null;
        return (program.days || []).some(d => d.id === overrideId) ? overrideId : null;
    }
    const legacy = Storage.getWorkoutOverrides()[dateStr];
    if (legacy && (program.days || []).some(d => d.id === legacy)) return legacy;
    return getScheduledWorkout(dateStr);
}

function getEffectiveWeekdayForSession(dayId, refDate) {
    for (const dateStr of getWeekDates(refDate)) {
        if (getEffectiveWorkout(dateStr) === dayId) return weekdayNameFromDate(dateStr);
    }
    const program = getTrainingProgram();
...
function applyWorkoutOverride(dateStr, dayId) {
    const weekStart = getWeekStartDate(dateStr);
    const dates = getWeekDates(dateStr);
    const base = {};
    dates.forEach(ds => { base[ds] = getScheduledWorkout(ds); });

    const saved = Storage.getWeekSwapMap(weekStart);
    const current = {};
    dates.forEach(ds => {
        current[ds] = Object.prototype.hasOwnProperty.call(saved, ds)
            ? (saved[ds] || null)
            : base[ds];
    });

    const alreadyHere = current[dateStr];
    if (alreadyHere === dayId) return;

    const program = getTrainingProgram();
    const session = (program.days || []).find(d => d.id === dayId);
    const incomingIsAlt = isAlternateDay(session);

    let sourceDate = dates.find(ds => current[ds] === dayId);
    if (!sourceDate && session && !incomingIsAlt) {
        sourceDate = dates.find(ds => weekdayNameFromDate(ds) === session.dayOfWeek);
    }

    current[dateStr] = dayId || null;
    // Alternates have no weekday slot — the replaced cycle day sits out this week.
    if (!incomingIsAlt && sourceDate && sourceDate !== dateStr) {
        current[sourceDate] = alreadyHere || null;
    }

    const deviations = {};
    dates.forEach(ds => {
        const next = current[ds] || null;
        const orig = base[ds] || null;
        if (next !== orig) deviations[ds] = next || '';
    });
    Storage.setWeekSwapMap(weekStart, deviations);
}

function renderSessionChips(program, activeId, viewDate, clickFn) {
    if (!program.days || program.days.length === 0) return '';
    const todayId = getEffectiveWorkout(viewDate);
    const chip = (d) => {
        const isActive = d.id === activeId;
        const slotted = getEffectiveWeekdayForSession(d.id, viewDate);
        const shownWeekday = slotted || (isAlternateDay(d) ? 'Alt' : d.dayOfWeek);
        const isTodaySlot = d.id === todayId;
        const textColor = isActive ? (isLightColor(d.color) ? '#000' : '#fff') : '';
        return `<button type="button" class="day-btn ${isActive ? 'active' : ''} ${isTodaySlot && !isActive ? 'scheduled' : ''} ${isAlternateDay(d) && !slotted ? 'alt-day' : ''}"
                style="${isActive ? 'background:' + d.color + '; border-color:' + d.color + '; color:' + textColor : ''}"
                onclick="event.stopPropagation(); ${clickFn}('${d.id}')">
            ${shownWeekday.slice(0,3)}<br><span style="font-size:10px">${d.name}</span>
            ${isTodaySlot ? '<span class="day-today-dot"></span>' : ''}
        </button>`;
    };
    const cycle = getCycleDays(program).slice().sort((a, b) => weekdaySortKey(a) - weekdaySortKey(b));
    const alts = getAlternateDays(program);
    return `<div class="day-selector">${cycle.map(chip).join('')}</div>
        ${alts.length ? `<div class="day-selector alt-day-selector">${alts.map(chip).join('')}</div>` : ''}`;
}
...
function renderGym() {
    ...
    const scheduledWorkout = getScheduledWorkout(currentGymDate);
    const todaysWorkout = getEffectiveWorkout(currentGymDate);
    const overrideId = todaysWorkout !== scheduledWorkout ? todaysWorkout : null;
    ...
    const scheduledDay = scheduledWorkout ? program.days.find(d => d.id === scheduledWorkout) : null;
    const isOverride = !!(currentGymDay && currentGymDay !== scheduledWorkout);
    let scheduleIndicator = '';

    if (isTodayDate && currentGymDay) {
        const weekNote = isOverride ? ' <span style="color:var(--text-muted)">(this week only)</span>' : '';
        scheduleIndicator = `<div class="today-workout-banner"><span class="today-badge">📅 TODAY</span><span>It's ${selectedDayName} — <strong>${day.name}</strong>${weekNote}</span></div>`;
    } else if (isTodayDate && !currentGymDay) {
        scheduleIndicator = `<div class="rest-day-banner"><span class="rest-badge">😴 REST DAY</span><span>It's ${selectedDayName} — no session on the calendar</span></div>`;
    } else if (!isTodayDate) {
        scheduleIndicator = `<div class="past-date-banner"><span>📆 ${formatDate(currentGymDate)} (${selectedDayName})</span></div>`;
    }
    ...
    page.innerHTML = `
        ...
        ${scheduleIndicator}
        ${renderSessionChips(program, currentGymDay, currentGymDate, 'selectGymDay')}
        ...
        <div class="card" style="border-left: 3px solid ${day.color}; padding: 12px 14px;">
            ...
            <div style="font-size:12px; color:var(--text-muted)">${selectedDayName} · ${day.exercises.length} exercises${isOverride ? ' · this week only' : ''}</div>
        </div>
...
function onGymDateChange(newDate) {
    ...
    currentGymDay = getEffectiveWorkout(newDate);
    currentSubstitutions = {};
    renderGym();
}
...
function selectGymDay(dayId) {
    currentGymDay = dayId;
    currentSubstitutions = {};
    applyWorkoutOverride(currentGymDate, dayId);
    renderGym();
}

function clearGymDayOverride() {
    Storage.setWeekSwapMap(getWeekStartDate(currentGymDate), {});
    currentGymDay = getScheduledWorkout(currentGymDate);
    currentSubstitutions = {};
    renderGym();
}
