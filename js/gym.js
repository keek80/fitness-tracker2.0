...
function renderGym() {
    ...
    const isOverride = !!(overrideId && overrideId === currentGymDay && overrideId !== scheduledWorkout);
    const isScheduledDay = scheduledWorkout === currentGymDay;
    let scheduleIndicator = '';

    if (isOverride) {
        const scheduledLabel = scheduledDay ? scheduledDay.name : 'the scheduled session';
        scheduleIndicator = `<div class="swap-workout-banner">
            <span class="swap-badge">🔄 SWAPPED</span>
            <span>Doing <strong>${day.name}</strong> instead of ${scheduledLabel} on ${selectedDayName}.</span>
            <button class="swap-undo-btn" type="button" onclick="clearGymDayOverride()">Undo</button>
        </div>`;
    } else if (isTodayDate && isScheduledDay && scheduledWorkout) {
    ...
    } else if (isTodayDate && !scheduledWorkout && !currentGymDay) {
        scheduleIndicator = `<div class="rest-day-banner"><span class="rest-badge">😴 REST DAY</span><span>It's ${selectedDayName} — no workout scheduled. Pick any session below.</span></div>`;
    } else if (isTodayDate && !scheduledWorkout && currentGymDay) {
        scheduleIndicator = `<div class="swap-workout-banner">
            <span class="swap-badge">🔄 EXTRA</span>
            <span>Rest day on the calendar — logging <strong>${day.name}</strong> instead.</span>
            <button class="swap-undo-btn" type="button" onclick="clearGymDayOverride()">Undo</button>
        </div>`;
    } else if (!isTodayDate && isOverride) {
        scheduleIndicator = `<div class="swap-workout-banner">
            <span class="swap-badge">🔄 SWAPPED</span>
            <span>Logging <strong>${day.name}</strong> for ${formatDate(currentGymDate)}.</span>
        </div>`;
    } else if (!isTodayDate) {
    ...
    page.innerHTML = `
        ...
        <div class="form-group" style="margin-bottom:8px">
            ...
            <div style="font-size:12px; color:var(--text-muted); margin-bottom:8px">
                Tap another day to train a different muscle group without changing your program.
            </div>
        </div>
        <div class="day-selector">
            ${program.days.map(d => {
                const isActive = d.id === currentGymDay;
                const isScheduled = d.id === scheduledWorkout;
