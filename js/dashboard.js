...
function renderDashboard() {
    ...
    const totalDays = (typeof getCycleDays === 'function' ? getCycleDays(program) : program.days).length;
    ...
    const sessionChips = (typeof renderSessionChips === 'function')
        ? renderSessionChips(program, todaysWorkoutDay ? todaysWorkoutDay.id : null, todayStr, 'pickDashboardWorkout')
        : '';
    ...
    if (todaysWorkoutDay) {
        const subtitle = isSwapped
            ? `${todayDayOfWeek} · ${todaysWorkoutDay.exercises.length} exercises · this week only`
        ...
        if (todayAlreadyLogged) {
            todayCard = `
                <div class="card" style="border-left: 3px solid var(--accent-green); padding: 12px 14px;">
                    ...
                    <div style="margin-top:12px">${sessionChips}</div>
                </div>
            `;
        } else {
            todayCard = `
