    ...
    page.innerHTML = `
        ...
        <div id="exerciseList">
            ${day.exercises.map((ex, i) => {
                ...
                const prBadge = pr ? `<span class="pr-badge">PR: ${pr.bestWeight}×${pr.bestReps}</span>` : '';

                // Get saved per-set weights (new format) or single weight (old format)
                const savedWeights = saved?.weights || (saved?.weight ? Array(ex.sets).fill(saved.weight) : Array(ex.sets).fill(''));
                const savedReps   = saved?.sets || Array(ex.sets).fill('');

                // Previous session display
                let prevDisplay = '';
                if (prev) {
                    const prevWeights = prev.weights || (prev.weight ? Array((prev.sets||[]).length).fill(prev.weight) : []);
                    ...
                    if (prevParts.length > 0) {
                        ...
                        prevDisplay = `
                            <div class="prev-session-info">
                                ...
                                ${prevMaxW > 0 ? `<button class="fill-btn" onclick="fillFromPrevious(${i}, [${prevWeights.join(',')}], [${prevSets.join(',')}])" title="Load previous session">⬆️ Load</button>` : ''}
                            </div>`;
                    }
                }
...
function saveGymLog() {
    ...
    const exercises = day.exercises.map((ex, i) => {
        ...
        // PR check: evaluate each set individually
        weights.forEach((w, s) => {
            const r = sets[s];
            if (w > 0 && r > 0) {
                const isPR = Storage.checkAndUpdatePR(ex.name, w, r);
                if (isPR) showToast(`🏆 New PR! ${ex.name}: ${w} lbs × ${r} reps`);
            }
        });
